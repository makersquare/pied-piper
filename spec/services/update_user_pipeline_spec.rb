...
describe UpdateUserPipeline do
  ...
  it "Updates a user in a pipeline" do
    ...
    result = UpdateUserPipeline.run(id: p1.data.id, user_id: user.id, pipeline_admin: false)

    expect(result.success?).to eq(true)
    expect(result.data.admin).to eq(false)

    RemoveUserPipeline.run(id: p1.data.id, user_id: user.id)
    result2 = UpdateUserPipeline.run(id: p1.data.id, user_id: user.id, pipeline_admin: false)
