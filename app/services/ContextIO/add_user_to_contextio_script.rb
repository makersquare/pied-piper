...
class AddUserToContextio < TransactionScript
  def run(user)
    ...
    ContextioHelper.create_new_source(account: account, email: user.email, server: 'imap.gmail.com', username: user.email, use_ssl: 1, port: 993, type:'IMAP', provider_refresh_token: user.oath_token)
