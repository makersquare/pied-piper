app.controller('PipelineDetailsCtrl',
  ...
  function($scope, $resource, $routeParams,
    ...
    $scope.contacts.$promise.then(function(data){
      $scope.contacts = data;
      return $scope.stages.$promise;
    }).then(function(data){
      $scope.stages = data;
      mapContactsToStages($scope.contacts, $scope.stages);
    });

    var mapContactsToStages = function(contacts, stages) {
...
// Edit the entry in the browser by double-clicking the text; press enter to update in the database
    ...
    $scope.changeStage = function(contact, stage){
      ...
      // ContactBoxRsc.update(contact).$promise.then(function(contact) {
