...
#     result = UpdateContactBox.run({:box_id=>b1.box.id, :contact_id=>c1.contact.id, :contact_name=>'updated contact name', :notes=>'updated notes'})
#     box = result.box
#     field = result.field
...
#     expect(contact.name).to eq('updated contact name')
...
#     expect(notes.first.value).to eq('updated notes')
#   end
